...
import { cid, isMessageSeenByOther, markOwnMessagesReadBy } from "../utils/chatRead";
...
const T = {
  ...
  ring: "focus:ring-pink-200 focus:border-pink-300",
  bubble: "bg-pink-500 text-white",
  bubbleOther: "bg-gray-100 text-gray-800",
  selected: "bg-pink-50",
  pin: "bg-pink-50 text-pink-800",
...
const ChatMessenger = () => {
  ...
  const otherUserId = selected?.participants?.find((p) => String(p.userId) !== String(user?._id))?.userId;
  ...
  const applyReadToOpenChat = useCallback(
    (conversationId, readerId, readAt) => {
      if (cid(readerId) === cid(user?._id)) return;
      if (cid(selectedRef.current) !== cid(conversationId)) return;
      setMessages((prev) => markOwnMessagesReadBy(prev, readerId, readAt, user?._id));
    },
    [user?._id]
  );
  ...
  useEffect(() => {
    ...
    socket.on("chat:message", ({ message, conversation }) => {
      ...
      if (isOpen) {
        ...
        if (cid(message.senderId) !== cid(user?._id)) {
      ...
      const merged = {
        ...conversation,
        lastMessageSeen: cid(message.senderId) === cid(user?._id) ? false : conversation.lastMessageSeen,
      };
    ...
    socket.on("chat:conversation-updated", ({ conversation }) => {
      ...
      if (conversation.lastMessageSeen) {
        const other = conversation.participants?.find((p) => cid(p.userId) !== cid(user?._id));
  ...
  }, [loadLists, refreshUnread, user?._id, applyReadToOpenChat]);
  ...
  const openConversation = async (conv) => {
    ...
    try {
      ...
      setMessages(items);
      setPinnedMessage(items.find((m) => m.isPinned && !m.isDeleted) || null);
      if (res.data.data?.conversation) setSelected(res.data.data.conversation);
      setConversations((prev) =>
        prev.map((c) => (c._id === conv._id ? { ...c, ...res.data.data.conversation, unreadCount: 0 } : c))
      );
      refreshUnread();
    } catch (e) {
      console.error(e);
  ...
  const isSeen = (msg) => isMessageSeenByOther(msg, messages, otherUserId, user?._id);
    ...
    if (msg.type === "image") {
      return (
        <button
          ...
          className="block mt-1 rounded-lg overflow-hidden max-w-full sm:max-w-[240px] focus:outline-none focus:ring-2 focus:ring-pink-300"
        >
          <img
            src={att.url}
    ...
    if (msg.type === "video") {
      return (
        <button
          type="button"
          onClick={() => openMedia(msg, att)}
          className="block mt-1 rounded-lg overflow-hidden max-w-full sm:max-w-[240px] relative focus:outline-none focus:ring-2 focus:ring-pink-300"
        >
    ...
    return (
      <button
        ...
        className={`flex items-center gap-2 mt-1 p-2.5 rounded-lg text-left w-full max-w-full sm:max-w-xs ${
          String(msg.senderId) === String(user?._id) ? "bg-white/15 hover:bg-white/25" : "bg-gray-200/80 hover:bg-gray-200"
        }`}
      >
  ...
  const convTitle = (conv) =>
    conv.participants?.filter((p) => String(p.userId) !== String(user?._id)).map((p) => p.userName).join(", ") || "Chat";
            ...
            {loadingList ? (
              ...
              <>
                {conversations.map((conv) => {
                  ...
                  const iSentLast = String(conv.lastMessage?.senderId) === String(user?._id);
                      ...
                      <div className="flex justify-between items-start gap-2">
                        ...
                        <div className="flex flex-col items-end shrink-0 gap-1">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {hasUnread && (
                              <span className="min-w-[18px] h-[18px] px-1 bg-pink-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">
        ...
        <div className={`${selected ? "flex" : "hidden md:flex"} flex-1 flex-col min-w-0 bg-white min-h-0`}>
          {selected ? (
            <>
              ...
              {replyTo && (
                <div className="px-2 sm:px-3 py-2 bg-pink-50 flex items-center gap-2 text-xs shrink-0">
...
const MsgBubble = ({ msg, isOwn, seen, menuOpen, onMenu, onReply, onForward, onEdit, onDelete, onPin, editing, editText, setEditText, onSaveEdit, onCancelEdit, renderAttachment, T }) => {
  if (msg.isDeleted) return <div className={`flex ${isOwn ? "justify-end" : "justify-start"}`}><p className="text-xs italic text-gray-400">Message deleted</p></div>;

  return (
    <div className={`flex gap-1 ${isOwn ? "justify-end" : "justify-start"} group`}>
      <div className="max-w-[88%] sm:max-w-[75%]">
        {!isOwn && <p className="text-[10px] text-gray-400 mb-0.5">{msg.senderName}</p>}
        {msg.forwardedFrom && <p className="text-[10px] text-gray-400 mb-0.5">Forwarded from {msg.forwardedFrom.senderName}</p>}
        {msg.replyTo && (
          <div className="text-[10px] bg-black/5 rounded px-2 py-1 mb-1 border-l-2 border-pink-400">
            <span className="font-medium">{msg.replyTo.senderName}</span>: {msg.replyTo.content}
          </div>
        )}
        <div className={`px-3 py-2 rounded-2xl text-sm ${isOwn ? `${T.bubble} rounded-br-md` : `${T.bubbleOther} rounded-bl-md`}`}>
          {editing ? (
          ...
          <div className={`flex items-center gap-1 mt-1 text-[9px] ${isOwn ? "text-white/70 justify-end" : "text-gray-400"}`}>
            ...
            {isOwn && (seen ? <CheckCheck size={12} className="text-pink-200" title="Seen" /> : <Check size={12} title="Sent" />)}
          </div>
        </div>
      </div>
      {!editing && (
        <div className="relative self-center">
          ...
          {menuOpen && (
            <div className="absolute right-0 top-full z-10 w-36 bg-white border rounded-lg shadow-lg text-xs py-1">
