...
const T = {
  ...
  ring: "focus:ring-blue-200 focus:border-blue-300",
  bubble: "bg-blue-500 text-white",
  bubbleOther: "bg-gray-100 text-gray-800",
  selected: "bg-blue-50",
  pin: "bg-blue-50 text-blue-800",
    ...
    if (msg.type === "image") {
      return (
        <button
          ...
          className="block mt-1 rounded-lg overflow-hidden max-w-full sm:max-w-[240px] focus:outline-none focus:ring-2 focus:ring-blue-300"
        >
          <img
            src={att.url}
    ...
    if (msg.type === "video") {
      return (
        <button
          type="button"
          onClick={() => openMedia(msg, att)}
          className="block mt-1 rounded-lg overflow-hidden max-w-full sm:max-w-[240px] relative focus:outline-none focus:ring-2 focus:ring-blue-300"
        >
        ...
        <div className={`${selected ? "flex" : "hidden md:flex"} flex-1 flex-col min-w-0 bg-white min-h-0`}>
          {selected ? (
            <>
              ...
              {replyTo && (
                <div className="px-2 sm:px-3 py-2 bg-blue-50 flex items-center gap-2 text-xs shrink-0">
    ...
    <div className={`flex w-full ${isOwn ? "justify-end" : "justify-start"}`}>
      <div className={`flex items-end gap-1 max-w-[88%] sm:max-w-[75%] ${isOwn ? "flex-row-reverse" : "flex-row"} group`}>
        <div className="min-w-0">
          ...
          {msg.replyTo && (
            <div className={`text-[10px] bg-black/5 rounded px-2 py-1 mb-1 border-l-2 border-blue-400 ${isOwn ? "mr-1" : "ml-1"}`}>
