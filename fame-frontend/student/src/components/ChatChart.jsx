...
const THEMES = {
  emerald: { stroke: "#10b981", fill: "#10b981", grid: "#d1fae5" },
  violet: { stroke: "#8b5cf6", fill: "#8b5cf6", grid: "#ede9fe" },
};

const ChatChart = ({ chart, theme = "emerald" }) => {
  if (!chart?.data?.length) return null;

  const palette = chart.colors?.length ? chart.colors : [THEMES[theme]?.fill || "#6366f1"];
      ...
      return (
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={chart.data} margin={{ top: 8, right: 8, left: -12, bottom: 0 }}>
            ...
            <Tooltip contentStyle={tooltipStyle} />
            <Line type="monotone" dataKey={dataKey} stroke={colors.stroke} strokeWidth={2} dot={{ r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      );
    }
  ...
  return (
    <div className="mt-2 rounded-xl border border-gray-100 bg-white/90 p-3 shadow-sm">
